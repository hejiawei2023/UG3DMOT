# The code refers to https://github.com/hailanyi/3D-Multi-Object-Tracker
...
def convert_bbs_type(boxes, input_box_type):
    ...
    if input_box_type == "Kitti":  # (h,w,l,x,y,z,yaw) -> (x,y,z,yaw,l,w,h)
        ...
        for i in range(t_id):
            ...
            new_boxes[:, b_id + 4] = boxes[:, b_id + 2]
            new_boxes[:, b_id + 5] = boxes[:, b_id + 1]
            new_boxes[:, b_id + 6] = boxes[:, b_id + 0]
            new_boxes[:, b_id + 3] = (np.pi - boxes[:, b_id + 6]) + np.pi / 2
